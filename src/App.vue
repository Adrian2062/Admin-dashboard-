<template>
  <div class="flex h-screen bg-gray-50 text-gray-900">
    <!-- Sidebar -->
    <div :class="`${sidebarOpen ? 'w-64' : 'w-20'} bg-white transition-all duration-300 border-r border-gray-200 shadow-sm relative`">
      <div class="p-4 flex items-center justify-between">
        <h1 v-if="sidebarOpen" class="text-xl font-bold text-blue-600">Admin</h1>
      ...
      <nav class="mt-8 space-y-2 px-2">
        <div v-for="item in menuItems" :key="item.id">
          ...
          <!-- Nested Menu Example -->
          <div v-if="item.subMenu && activeTab === item.id" class="ml-8 mt-2 space-y-1">
            <button
      ...
      <div class="absolute bottom-4 left-2 right-2">
        <button class="w-full flex items-center gap-3 px-4 py-3 rounded-lg text-gray-600 hover:bg-gray-50 transition">
    ...
    <div class="flex-1 overflow-auto">
      ...
      <div class="bg-white border-b border-gray-200 px-8 py-4 flex justify-between items-center sticky top-0 z-10 shadow-sm">
        ...
        <div class="flex items-center gap-4">
          ...
          <div class="w-10 h-10 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center text-white font-bold">A</div>
        </div>
      </div>
      ...
      <div class="p-8">
        ...
        <div v-if="activeTab === 'home'" class="space-y-8">
          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            <div v-for="(stat, idx) in stats" :key="idx" class="bg-white border border-gray-200 rounded-lg p-6 hover:shadow-lg transition shadow-sm">
              <div :class="`w-12 h-12 ${stat.color} rounded-lg mb-4 flex items-center justify-center`">
              ...
              <p :class="`text-sm mt-2 font-medium ${stat.change.startsWith('+') ? 'text-green-600' : 'text-red-600'}`">
                {{ stat.change }} from last month
              </p>
              <!-- Optional progress bar -->
              <div class="w-full bg-gray-100 h-2 rounded-full mt-2">
                <div :style="{ width: stat.progress + '%' }" class="h-2 rounded-full bg-gradient-to-r from-blue-400 to-blue-600"></div>
              </div>
            </div>
          </div>
          ...
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
              ...
              <div class="h-64 bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg flex items-center justify-center text-gray-500">
          ...
          <div class="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
            <div class="p-6 border-b border-gray-200 flex justify-between items-center">
              ...
              <input type="text" placeholder="Search..." v-model="userSearch" class="px-3 py-2 border rounded-lg text-sm" />
            </div>
            <div class="overflow-x-auto">
              <table class="w-full">
                <thead class="bg-gray-50 text-left text-sm font-semibold border-b border-gray-200">
                  <tr>
                ...
                <tbody class="divide-y divide-gray-200">
                  <tr v-for="user in filteredUsers" :key="user.id" class="hover:bg-gray-50 transition">
                    ...
                    <td class="px-6 py-4 flex gap-2">
                      <button class="px-2 py-1 text-sm bg-blue-500 text-white rounded hover:bg-blue-600">Edit</button>
                      <button class="px-2 py-1 text-sm bg-red-500 text-white rounded hover:bg-red-600">Delete</button>
                    </td>
                  </tr>
                </tbody>
              </table>
        ...
        <div v-else-if="activeTab === 'analytics'" class="space-y-6">
          <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
            ...
            <p class="text-gray-600">Graphs, KPIs, and trends would appear here.</p>
          </div>
        </div>
        ...
        <div v-else-if="activeTab === 'settings'" class="space-y-6">
          <div class="bg-white border border-gray-200 rounded-lg p-6 shadow-sm">
            ...
            <p class="text-gray-600">Theme, notifications, profile settings, etc.</p>
          </div>
        </div>
      </div>
    </div>
...
const menuItems = [
  ...
  { id: 'users', label: 'Users', icon: 'UsersIcon', subMenu: [{ id: 'user-list', label: 'User List' }, { id: 'roles', label: 'Roles' }] },
...
// Stats
const stats = [
  { label: 'Total Users', value: '12,345', change: '+12%', color: 'bg-blue-500', progress: 80 },
  { label: 'Revenue', value: '$45,231', change: '+8%', color: 'bg-green-500', progress: 65 },
  { label: 'Orders', value: '2,543', change: '-3%', color: 'bg-purple-500', progress: 45 },
  { label: 'Conversion', value: '3.24%', change: '+2%', color: 'bg-orange-500', progress: 70 },
]
...
const recentUsers = [
  ...
  { id: 4, name: 'James Wilson', email: 'james@example.com', role: 'Editor', status: 'Active' },
]
...
const filteredUsers = computed(() => {
  ...
  return recentUsers.filter(u => u.name.toLowerCase().includes(userSearch.value.toLowerCase()))
})
</script>
